...
namespace YOLOv4 {
  ...
  Odin::addDisplayMeta(gpointer batch_meta_data, gpointer frame_meta_data, int status) {
    ...
    //   g_printerr("%d", shot_count);
    // }
    offset = snprintf(txt_params->display_text, MAX_DISPLAY_LEN, "Penalty Count:%d",status);
    // else if(status==-1)
    ...
    txt_params->text_bg_clr.alpha = 1.0;
      ...
      // 4 ROI line coordinates 
      // NvOSD_LineParams *line_params  = display_meta->line_params; 
      // [(615 (x1), 145 (y1)), (1373 (x2), 433 (y2))]
      // for demonstration, user need to se these values
      // Ball.MP4 First Source Video Coordinates
       line_params[0].x1 = 615;
       line_params[0].y1 = 200;
       line_params[0].x2 = 1373; 
       line_params[0].y2 = 200;  
       line_params[0].line_width = 4;
       line_params[0].line_color = (NvOSD_ColorParams){0.0, 0.1, 1.0, 1.0};
       display_meta->num_lines++;

        line_params[1].x1 = 1373;//for demonstration, user need to se these values
       line_params[1].y1 = 200;
       line_params[1].x2 = 1373; 
       line_params[1].y2 = 458;
       line_params[1].line_width = 4;
       line_params[1].line_color = (NvOSD_ColorParams){0.0, 0.0, 1.0, 1.0};
       display_meta->num_lines++;

        line_params[2].x1 = 1373;//for demonstration, user need to se these values
       line_params[2].y1 = 458;
       line_params[2].x2 = 615;
       line_params[2].y2 = 458; 
       line_params[2].line_width = 4;
       line_params[2].line_color = (NvOSD_ColorParams){0.0, 0.0, 1.0, 1.0};
       display_meta->num_lines++;
       
        line_params[3].x1 = 615;//for demonstration, user need to se these values
       line_params[3].y1 = 458;
       line_params[3].x2 = 615;
       line_params[3].y2 = 200;    
       line_params[3].line_width = 4;
       line_params[3].line_color = (NvOSD_ColorParams){0.0, 0.0, 1.0, 1.0};
       display_meta->num_lines++;

       // Second Source Video Corrdinates (Balls.MP4)

      //  line_params[0].x1 = 440;//for demonstration, user need to se these values
      //  line_params[0].y1 = 188;
      //  line_params[0].x2 = 1475; 
      //  line_params[0].y2 = 188;  
      //  line_params[0].line_width = 4;
      //  line_params[0].line_color = (NvOSD_ColorParams){0.0, 0.1, 1.0, 1.0};
      //  display_meta->num_lines++;

      //   line_params[1].x1 = 1475;//for demonstration, user need to se these values
      //  line_params[1].y1 = 188;
      //  line_params[1].x2 = 1475; 
      //  line_params[1].y2 = 522;
      //  line_params[1].line_width = 4;
      //  line_params[1].line_color = (NvOSD_ColorParams){0.0, 0.0, 1.0, 1.0};
      //  display_meta->num_lines++;

      //   line_params[2].x1 = 1475;//for demonstration, user need to se these values
      //  line_params[2].y1 = 522;
      //  line_params[2].x2 = 440;
      //  line_params[2].y2 = 522; 
      //  line_params[2].line_width = 4;
      //  line_params[2].line_color = (NvOSD_ColorParams){0.0, 0.0, 1.0, 1.0};
      //  display_meta->num_lines++;
       
      //   line_params[3].x1 = 440;//for demonstration, user need to se these values
      //  line_params[3].y1 = 522;
      //  line_params[3].x2 = 440;
      //  line_params[3].y2 = 188;    
      //  line_params[3].line_width = 4;
      //  line_params[3].line_color = (NvOSD_ColorParams){0.0, 0.0, 1.0, 1.0};
      //  display_meta->num_lines++;
  ...
  Odin::tiler_src_pad_buffer_probe(GstPad *pad, GstPadProbeInfo *info,
    ...
    for (l_frame = batch_meta->frame_meta_list; l_frame != NULL;
      ...
      data.emplace_back(cv::Point2f(615, 200));
      data.emplace_back(cv::Point2f(1373, 200));
      data.emplace_back(cv::Point2f(1373, 458));
      data.emplace_back(cv::Point2f(615, 458));


      // Second Source Video Corrdinates (Balls.MP4)

      // data.emplace_back(cv::Point2f(440, 188));
      // data.emplace_back(cv::Point2f(1475, 188));
      // data.emplace_back(cv::Point2f(1475, 522));
      // data.emplace_back(cv::Point2f(440, 522));
  ...
  void Odin::setPaths(guint batch_size) {
    ...
    str(boost::format("models/YOLOv4/model_b%d_gpu0_%s.engine") % batch_size % COMPUTE_MODE);
  }
}
